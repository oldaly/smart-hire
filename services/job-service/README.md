...
```bash
job-service$ sam local invoke helloFromLambdaFunction --no-event
```
...
```bash
job-service$ sam logs -n helloFromLambdaFunction --stack-name sam-app --tail
```
...
```bash
job-service$ dotnet test test/job-service.Tests/job-service.Tests.csproj 
```
...
```bash
sam delete --stack-name job-service
```
